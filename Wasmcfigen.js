...
class Wasmcfigen {
  constructor(wabtBinPath, functionSignatureFilePath, watPath) {
    ...
    this.randIndexes = [];
    this.indCallCount = 0;
    this.funcInElemSections = [];

    try {
      callSiteObj = JSON.parse(
        `{${fs.readFileSync(this.externPaths[`funcSigPath`], "utf-8")}}`
      );
      this.#runFuncChains(
        this.#filterIndCalleeSync,
        this.#sortFuncsByTypeValue,
        this.#setIndCallSigs
      )(this.externPaths[`watPath`]);
    } catch (err) {
  ...
  #filterIndCalleeSync = (watPath) => {
    this.funcInElemSections = this.#readElemSync(watPath);
    this.indCallCount = this.funcInElemSections.length;
    let indCallObj = {};

    this.funcInElemSections.forEach((funcName, tableIdx) => {
      funcName = funcName.slice(1);
      indCallObj[funcName] = {};
      indCallObj[funcName].Type = callSiteObj[funcName]
        ? callSiteObj[funcName].Type
        : { ret: "", args: [] };
  ...
  #readElemSync = (watPath) => {
    ...
    this.externPaths[`wasmPath`] = `${dirPath}/${
      watFileName.split(".")[0]
    }.wasm`;
  ...
  #createWat = (wasmPath, watPath) =>
    this.#exec(
      `${this.externPaths["wabtPath"]}/wasm2wat ${wasmPath} -o ${watPath}`
    );
  ...
  indexRandomize = () => {
    let indexPairs = [];
    Object.values(this.indCallSigs).reduce((acc, sigInfo) => {
  ...
  modWatSync = () => {
    ...
    try {
      ...
      this.#exec(
        `${this.externPaths[`wabtPath`]}/wat2wasm ${watPath} -o  ${
          this.externPaths[`wasmPath`]
        }`
      );
    } catch (err) {
  ...
  #renewElemAndIndexSection = (watPath) => {
    ...
    const newDataSectionStr = this.#getNewIndexesArray(this.randIndexes);
  ...
  #getNewIndexesArray = (idxArr) => {
    const hexValStr = idxArr.reduce(
      (pre, newIdxStr) =>
        pre + this.#toLittleEndian(parseInt(newIdxStr).toString(16)),
      ""
    );
  ...
  #modElemSection = () => {
    ...
    let newElemFuncArr = Array.of(this.indCallCount);
  ...
  #modDataSection = () => {
    ...
    const isFromExternalLib = (func) => !callSiteObj[func];
    const hasIndCall = (func) => callSiteObj[func].Calls.length > 0;
      ...
      if (isFromExternalLib(fName)) {
        ...
        callSiteObj[fName].Calls.forEach((call) => {
          let typeValueStr = this.#getSigVal(
            this.#getTypeVal(call.ret),
            call.args.map((argType) => this.#getTypeVal(argType))
          );
    ...
    const globalForOriginalIdx = `\n\t(global (mut i32) (i32.const ${
      originalOffset + 4 * this.indCallCount
    })))`;
  ...
  #setIndexBoundaries = () => {
    ...
    let acc = 1;
    Object.entries(this.indCallSigs).forEach((sigEntry) => {
  ...
  #getIndexRangeCheckStmt = (idxOffset, lowerbound, upperbound) => {
  ...
  getIndCallSigs = () => {
    const signatureMsg = Object.entries(this.indCallSigs).reduce(
      (pre, [sig, detail]) =>
        pre +
        `${sig} : {\tfuncMem : {\n` +
...
let cfi = new Wasmcfigen(...process.argv.slice(2));
cfi.modWatSync();
cfi.getIndCallSigs();
